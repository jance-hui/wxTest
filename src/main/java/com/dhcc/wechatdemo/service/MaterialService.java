...
import com.dhcc.wechatdemo.entity.material.ArticlesUpdate;
...
public class MaterialService {
     ...
     * 修改永久图文素材
     * @param accessToken accessToken
     * @param update 要更新的图文文章信息
     * @return JSONObject
     */
    public JSONObject updateNews(String accessToken, ArticlesUpdate update){
        // 设置url
        String url = MaterialConstants.UPDATE_NEWS.replace("ACCESS_TOKEN", accessToken);
        JSONObject jsonObject = new JSONObject();
        try {
            logger.info("开始修改永久图文素材了，素材id：" + update.getMedia_id());
            logger.info("修改第 " + update.getIndex()+1 + " 条文章");
            jsonObject = HttpHelper.doPost(url, update);
            logger.info("修改永久图文素材成功");
        } catch (Exception e) {
            logger.error("修改永久图文素材失败");
            e.printStackTrace();
        }
        return jsonObject;
    }
